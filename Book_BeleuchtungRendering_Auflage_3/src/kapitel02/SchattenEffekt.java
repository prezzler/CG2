...
public class SchattenEffekt extends LWJGLBasisFenster {
	...
	public static String fragShaderShadowVisualisation = "" 
		+ "#version 130\n"
		+ ""
		+ "uniform sampler2D tex1;" 
		+ "" 
	...
	private int programWaterdynamic, programShadowVisualisation;
	private int uniform_fragShader_waterDynamic_s, uniform_fragShader_waterDynamic_tex1, uniform_fragShader_waterDynamic_tex2;
	private int uniform_programShadowVisualisation_scale, uniform_programShadowVisualisation_s, uniform_programShadowVisualisation_tex1;
	
	public SchattenEffekt(String title, int width, int height) {
	...
	public void prepareShaderVisualisierung() {
		programShadowVisualisation = glCreateProgram();

		int fragShader = glCreateShader(GL_FRAGMENT_SHADER);
		glShaderSource(fragShader, fragShaderShadowVisualisation);
		glCompileShader(fragShader);
		System.out.println(glGetShaderInfoLog(fragShader, 1024));
		glAttachShader(programShadowVisualisation, fragShader);

		glLinkProgram(programShadowVisualisation);
		uniform_programShadowVisualisation_s 	= glGetUniformLocation(programShadowVisualisation, "s");
		uniform_programShadowVisualisation_scale 	= glGetUniformLocation(programShadowVisualisation, "scale");
		uniform_programShadowVisualisation_tex1 	= glGetUniformLocation(programShadowVisualisation, "tex1");
		glUseProgram(programShadowVisualisation);
		
		glUniform2f(uniform_programShadowVisualisation_s, 1.0f/WB, 1.0f/HB);			
		glUniform1f(uniform_programShadowVisualisation_scale, -100.0f);
		glUniform1i(uniform_programShadowVisualisation_tex1, 0);
		
		ShaderUtilities.testShaderProgram(programShadowVisualisation);
	}
	...
	public static void main(String[] args) throws LWJGLException {
		new SchattenEffekt("vividus Verlag. Dino-Buch. Kapitel 2: SchattenEffekt.java", 800, 450).start();
	}
}