...
public class TorusSpecularDirectionalLight extends LWJGLBasisFenster {
   private int uniform_specularWeight, uniform_specularPow;
   private JSlider slider, sliderPow;
   private final static int sliderMAXI = 100;

   public TorusSpecularDirectionalLight(String title, int width, int height) {
   ...
   private void prepareShader() {
      ...
      try {
         vertexShaderSource = ShaderUtilities.readShadercode("shader/torusspeculardirectionallight.vs");
      } catch (Exception e) {
         e.printStackTrace();
      }
      ...
      try {
         fragShaderSource = ShaderUtilities.readShadercode("shader/torusspeculardirectionallight.fs");
      } catch (Exception e) {
         e.printStackTrace();
      }
   ...
   public void renderLoop() {
      ...
      while (!Display.isCloseRequested()) {
         ...
         glUniform1f(uniform_specularWeight, slider.getValue() / 100.0f);
         glUniform1f(uniform_specularPow, sliderPow.getValue());
   ...
   public static void main(String[] args) throws Exception {
      new TorusSpecularDirectionalLight("vividus Verlag. Dino-Buch. Kapitel 5: TorusSpecularDirectionalLight.java", 800,
            450).start();
   }
}