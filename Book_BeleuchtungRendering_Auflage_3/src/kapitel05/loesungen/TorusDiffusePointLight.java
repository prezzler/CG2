...
public class TorusDiffusePointLight extends LWJGLBasisFenster {
   private int diffusePercent;
   private JSlider slider;
   private final static int sliderMAXI = 100;

   public TorusDiffusePointLight(String title, int width, int height) {
   ...
   private void prepareShader() {
      ...
      try {
         vertexShaderSource = ShaderUtilities.readShadercode("shader/torusdiffusepointlight.vs");
      } catch (Exception e) {
         e.printStackTrace();
      }
      ...
      try {
         fragShaderSource = ShaderUtilities.readShadercode("shader/torusdiffusepointlight.fs");
      } catch (Exception e) {
         e.printStackTrace();
      }
      ...
      diffusePercent = glGetUniformLocation(myProgram, "diffuseWeight");
   ...
   public void renderLoop() {
      ...
      while (!Display.isCloseRequested()) {
         ...
         glUniform1f(diffusePercent, slider.getValue() / 100.0f);
   ...
   public static void main(String[] args) throws Exception {
      new TorusDiffusePointLight("vividus Verlag. Dino-Buch. Kapitel 5: TorusDiffusePointLight.java", 800, 450).start();
   }
}
