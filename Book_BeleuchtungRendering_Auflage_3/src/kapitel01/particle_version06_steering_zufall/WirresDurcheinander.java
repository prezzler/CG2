...
public class WirresDurcheinander extends LWJGLBasisFenster {
   private ObjektManager flummis;

   public WirresDurcheinander(String title, int width, int height) {
   ...
   private void erzeugeFlummies(int anz) {
      ...
      for (int i = 0; i < anz; i++) {
         Flummi flummi = new Flummi(new Vektor2D(rand.nextInt(WIDTH), rand.nextInt(HEIGHT)),
               ...
               rand.nextFloat());
         flummi.setVerhalten(new VerhaltenZufaellig(flummi));
   ...
   public static void main(String[] args) {
      new WirresDurcheinander(
            "vividus Verlag. Dino-Buch. Kap. 1 (particle_version06_steering_zufall): WirresDurcheinander.java", 800,
            450).start();
   }
}
