...
public class LineareAlgebra {
	...
	public static Vektor3D cartesianToPolar(Vektor3D v) {
		double length = v.length();
		return new Vektor3D(length, Math.atan2(v.y, v.x), Math.acos(v.z/length)); 
	}
	
	public static Vektor3D cartesianNormToPolar(Vektor3D v) {
		return new Vektor3D(1.0f, Math.atan2(v.y, v.x), Math.acos(v.z)); 
	}
	
	public static Vektor3D polarToCartesian(Vektor3D p) {
		return new Vektor3D(p.x*Math.sin(p.z) * Math.cos(p.y), 
				            p.x*Math.sin(p.z) * Math.sin(p.y), 
				            p.x*Math.cos(p.z));
	}
	
	public static Vektor3D polarToCartesianNorm(Vektor3D p) {
		return new Vektor3D(Math.sin(p.z) * Math.cos(p.y), Math.sin(p.z) * Math.sin(p.y), Math.cos(p.z));
	}
